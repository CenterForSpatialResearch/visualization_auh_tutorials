...
function updateBarChart(dataRow,color){
    d3.selectAll(".bar")
        ...
        .attr("fill",color)
        .attr("rx",function(d){return 0})//need to make the corner radius into 0 to be rect
        .attr("ry",function(d){return 0})//need to make the corner radius into 0 to be rect
        .attr("opacity",1)
}

function barToScatterPlot(dataRowX, dataRowY, dataRowR, color){
    d3.selectAll(".bar")
        .transition()
        .duration(2000)
        .attr("y",function(d){return publicData[dataRowY][d]*3})
        .attr("x",function(d){return publicData[dataRowX][d]*3})
        .attr("width",function(d){return publicData[dataRowR][d]})
        .attr("height",function(d){return publicData[dataRowR][d]})
        .attr("rx",function(d){return publicData[dataRowR][d]/2})//changes the corner radius of x
        .attr("ry",function(d){return publicData[dataRowR][d]/2})//changes the corner radius of y
        .attr("fill",color)
        .attr("opacity",.6)
}
